...
type User = {
  ...
  roleId: number;
  profile?: string | null;
  customAddressWalletsPairFields: any;
  customRestrictionPairFields: any;
  metadata: {
    location: {
      address: string;
      country: string;
  ...
  immer((set, get) => {
    ...
    return {
      ...
      filterMenu: (menu) => {
        ...
        if (!settings) return []; // Ensure settings and extensions are available
      ...
      fetchProfile: debounce(async () => {
        ...
        if (!error && data) {
          ...
          const hasAdminAccess = adminMenu.some((menuItem: IMenu) =>
            checkPermission(menuItem.permission, profileData.role.permissions)
          );
      ...
      fetchSettings: async () => {
        ...
        if (data && !error) {
          ...
          set((state) => {
            state.extensions = settingsData.extensions;
            state.settings = settingsData.settings.reduce((acc, setting) => {
