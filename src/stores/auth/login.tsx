  ...
  immer((set, get) => {
    ...
    return {
      ...
      handleSubmit: async (router) => {
        ...
        if (
          loginData.message &&
          loginData.message.startsWith("User email not verified")
        ) {
          set((state) => {
          ...
          loginData.message &&
          loginData.message.includes("2FA required")
        ) {
          set((state) => {
            state.is2FAVerificationStep = true;
            state.userId = loginData.id;
            state.twoFactorType = loginData.twoFactor.type;
          });
        } else {
