...
const useP2PStore = create<TradeState>()(
  immer((set, get) => ({
    ...
    checkIfExistActiveTrade: async () => {
      const url = "/api/ext/p2p/trade/status";
      try {
        const { data, error } = await $fetch({
          url,
          silent: true,
    ...
    replyToTrade: async (message, attachment) => {
      ...
      console.log("resp from replyToTrade", resp);
      ws?.send({
        payload: {
          id: get().trade?.id,
          message: messageData,
    ...
    addTradeMessage: async (message: string) => {
      const { trade } = get();
      if (!trade) return;
      await $fetch({
        url: `/api/ext/p2p/trade/${trade.id}/message`,
        method: "POST",
        body: { message },
      });
    },
