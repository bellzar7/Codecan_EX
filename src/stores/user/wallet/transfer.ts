...
export const useTransferStore = create<TransferStore>()(
  immer((set, get) => ({
    ...
    initializeWalletTypes: () => {
      ...
      // ECO (Ecosystem/Funding) - based on extension
      if (ecosystemEnabled) {
        walletTypes.push({ value: "ECO", label: "Funding" });
      }

      // FIAT - prepend if enabled
      if (fiatWalletsEnabled) {
        walletTypes.unshift({ value: "FIAT", label: "Fiat" });
      }

      // FUTURES - based on extension
      if (futuresEnabled) {
        walletTypes.push({ value: "FUTURES", label: "Futures" });
      }

      // Trading wallet types (always available)
      // These are internal-only wallets funded via transfers
      walletTypes.push({ value: "FOREX", label: "Forex" });
      walletTypes.push({ value: "STOCK", label: "Stock" });
      walletTypes.push({ value: "INDEX", label: "Index" });
