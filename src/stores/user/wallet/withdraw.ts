...
export const useWithdrawStore = create<WithdrawStore>()(
  immer((set, get) => ({
    ...
    initializeWalletTypes: () => {
      ...
      // Only wallet types that support direct withdraw are included here
      // FUTURES, FOREX, STOCK, INDEX are internal-only and MUST NOT appear in withdraw flow
      const walletTypes = [{ value: "SPOT", label: "Spot" }];

      // ECO (Ecosystem) - supports withdraw if extension is enabled
      if (ecosystemEnabled) {
        walletTypes.push({ value: "ECO", label: "Funding" });
      }

      // FIAT - supports withdraw if fiat wallets are enabled
    ...
    fetchWithdrawMethods: async () => {
      ...
      try {
        ...
        if (error) {
          ...
          set((state) => {
            state.withdrawMethods = methodsData;
            state.step = 3;
          });
        }
      } catch (error) {
