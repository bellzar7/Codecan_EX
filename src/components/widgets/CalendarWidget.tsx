...
const calendarStyles = cva(
  "mx-auto flex w-full items-center justify-center rounded-lg py-2.5 text-xs transition-colors duration-300",
  {
    variants: {
      isToday: { true: "", false: "" },
    ...
    compoundVariants: [
      ...
      {
        isToday: true,
        className: "bg-primary-500/10 font-semibold text-primary-500",
      },
      {
        isSelected: true,
        isToday: true,
  ...
  return (
    <div className="w-full">
      ...
      <div className="w-full text-muted-800 text-xs dark:text-muted-100">
        <div className="grid grid-cols-7 font-semibold text-muted-500 text-xs uppercase *:py-2 *:text-center">
          <div>{t("Sun")}</div>
          <div>{t("Mon")}</div>
          <div>{t("Tue")}</div>
          <div>{t("Wed")}</div>
        ...
        <div className="grid grid-cols-7 gap-[0.2rem] text-sm">
          {days.map((day, dayIdx) => (
            <div
              className={`  ${
