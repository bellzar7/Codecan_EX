...
const ReferralTreeBase = ({ id, isAdmin = false }: Props) => {
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {selectedUser && (
          <motion.div
            ...
            className="absolute top-3 left-3 z-50 mr-5 text-sm"
            exit={{ opacity: 0, scale: 0.9 }}
            initial={{ opacity: 0, scale: 0.9 }}
            key="selected-user-card"
          >
            <Card className="w-auto px-4 pt-6 pb-4" color="muted">
              <Icon
                className="absolute top-2 right-2 h-4 w-4 cursor-pointer text-gray-500 hover:text-danger-500 dark:text-gray-400 dark:hover:text-danger-500"
      ...
      <div
        className="relative z-0 h-[60vh] w-full overflow-hidden rounded-2xl"
