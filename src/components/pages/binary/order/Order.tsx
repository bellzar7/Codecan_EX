...
const BINARY_PROFIT = Number.parseFloat(
...
const OrderBase = ({}: OrderProps) => {
  ...
  const canPlaceOrder = useCallback(() => {
    ...
    if (getSetting("binaryRestrictions") === "true") {
      const kycLevel = Number.parseFloat(profile?.kyc?.level || "0");
  ...
  return (
    <>
      <div className="flex flex-col justify-between gap-3 p-4 md:overflow-x-auto md:p-2">
