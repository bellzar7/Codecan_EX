...
const RangeSlider: FC<RangeSliderProps> = ({
  ...
  return (
    <div
      className={`flex items-center text-muted-500 text-sm ${legend ? "justify-between gap-5" : ""}
      ${orientation === "vertical" ? "min-h-[160px] w-6 flex-col" : "w-full"}
    `}
    >
      {legend ? (
        <span className="relative top-0.5 left-2 font-sans text-muted-500 text-sm leading-none dark:text-muted-400">
          <span className="pe-1">
            {valuePrefix}
            {min}
          </span>
      ...
      <ReactSlider
        className={`relative ${orientation === "vertical" ? "min-h-[160px]" : "w-full"}
        `}
        disabled={disabled}
        invert={invert}
        ...
        renderThumb={(props, state) => {
          ...
          return (
            <div
              className={`absolute -top-1 left-0 rotate-45 rounded-sm ${color === "primary" ? "border-primary-500" : ""}
            ...
            >
              <div
                className={`absolute top-0.5 -right-3 bottom-0 my-auto ${thumbClass} rotate-45 cursor-pointer rounded-sm ring-offset-2 ring-offset-white dark:ring-offset-muted-950 ${
              ...
              >
                {tooltip ? (
                  <div className="relative h-full w-full -rotate-45 opacity-0 transition-opacity duration-200">
                    <div className="absolute top-[-1.22rem] left-[0.11rem] h-3 w-3 translate-y-2 -rotate-45 bg-muted-900 transition-all duration-300 ease-in-out dark:bg-muted-700" />
                    <div className="absolute -top-12 left-[-0.4rem] flex h-8 w-8 translate-y-2 items-center justify-center rounded-full bg-muted-900 text-center font-medium font-sans text-[0.52rem] text-white transition-all duration-300 ease-in-out dark:bg-muted-700">
      ...
      />
      {legend ? (
        <span className="relative top-0.5 font-sans text-muted-500 text-sm leading-none dark:text-muted-400">
          <span
            className={`relative ${
