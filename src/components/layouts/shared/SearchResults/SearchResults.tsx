...
const SearchResultsBase = ({ searchTerm, id }: SearchResultsProps) => {
    ...
    if (result.icon && !result.photoUrl) {
      return (
        <div className={iconClass}>
          <Icon className="h-4 w-4" icon={result.icon} />
        </div>
      );
    }
  ...
  const renderSubMenu = (result: any, i: number) => (
    <div className="ml-4 rounded-b-lg border-muted-200 border-x border-b bg-muted-50 dark:border-muted-800 dark:bg-muted-900">
      {result.subMenu.map((sub: any, j: number) => (
        ...
        >
          {sub.icon && (
            <div className="mask mask-blob flex h-9 w-9 items-center justify-center bg-primary-500/10 font-bold text-primary-500 text-xs dark:bg-primary-500/20">
              <Icon className="h-4 w-4" icon={sub.icon} />
            </div>
          )}
          <div className="font-sans">
            <span className="block font-medium text-muted-800 text-sm leading-tight dark:text-muted-100">
              {sub.title}
            </span>
          </div>
      ...
      <div key={i}>
        <a
          className={`group flex items-center gap-2 p-2 transition-all duration-100 hover:bg-muted-100 dark:hover:bg-muted-850 ${
            activeIndex === i
              ? "rounded-t-lg rounded-bl-lg border-muted-200 border-x border-t bg-muted-100 dark:border-muted-700 dark:bg-muted-950"
              : "rounded-lg"
          }`}
          href={result.href}
          onClick={(e) => {
  ...
  return (
    <div
      className={`slimscroll absolute top-12 left-0 z-5 max-h-[285px] w-full overflow-y-auto rounded-lg border border-muted-200 bg-white p-2 shadow-lg shadow-muted-300/30 transition-all duration-300 dark:border-muted-800 dark:bg-muted-900 dark:shadow-muted-800/30 ${
        searchTerm.length > 0
      ...
      {searchResults.length > 0 ? (
        ...
        <div className="flex min-h-[255px] items-center justify-center px-8">
          <div className="text-center font-sans">
            ...
            <p className="mx-auto max-w-[420px] text-muted-400 text-sm">
              {t(
