...
const TopBarBase = ({
  ...
  return (
    <ul>
      <li
        className={
          "flex min-h-[52px] items-center justify-end border-muted-200 border-b px-4 dark:border-muted-800"
        }
      >
        <h2
          className={`me-auto font-light font-sans text-base uppercase leading-${
            float ? "[2]" : "normal"
          } tracking-${
            float ? "[3px]" : "widest"
        ...
        <a
          className={`relative flex h-10 w-10 cursor-pointer items-center justify-center p-0 text-center text-xl opacity-1 leading-${
            float ? "[38.4px]" : "tight"
              ...
              <i
                className={`absolute block h-0.5 w-5 bg-muted-400 transition-all duration-200 ease-in-out ${
                  sidebarOpened || isSidebarOpenedMobile
                    ? "top-3 left-1/2 -ms-2.5 -mt-0.5 rotate-45"
                    : "top=[5.4px] ms-2.5"
                } `}
              />
              <i
