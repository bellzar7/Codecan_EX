...
const SubMenuItemBase = ({
  ...
  const dynamicClass = cn(
    collapse
      ? "ms-6 me-12 min-h-[27.2px] border-primary-800 border-r-[2px] duration-300"
      : sideblock
        ? "ms-9 me-7 min-h-[27.2px] border-muted-100 border-r-[3px] dark:border-muted-800"
        : "ms-3 me-5 min-h-[24px] border-muted-200/80 border-r-[2px] duration-200 dark:border-muted-800/80",
  ...
  return (
    <li>
      <Link className={linkClass} href={href} onClick={handleClick}>
        {t(title)}
      </Link>
    </li>
  );
