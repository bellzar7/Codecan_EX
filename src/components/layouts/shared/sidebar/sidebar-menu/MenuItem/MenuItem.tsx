    ...
    >
      <button className={menuItemClass} onClick={menuToggle}>
        ...
        <div className="ml-2 flex flex-col items-start">
          ...
          {description && (
            <span className="text-muted-400 text-xs leading-none transition-all duration-300 ease-in-out group-hover/menu-item:text-muted-800 dark:text-muted-500 dark:group-hover/menu-item:text-muted-200">
              {t(description)}
            </span>
          )}
        </div>
        {collapse &&
          !href && ( // Only show chevron if collapse is true and href is not provided
            <span className={iconWrapperClass}>
              <Icon className={chevronClass} icon="feather:chevron-down" />
            </span>
          )}
        {!(collapse || href) && ( // Adjusted logic to not show chevron when href is provided
          <Icon className={chevronClass} icon="lucide:chevron-down" />
        )}
      </button>
