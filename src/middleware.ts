...
export async function middleware(request: NextRequest) {
  // Легкий дев-режим, щоб не блокувати рендер
  if (process.env.NEXT_PUBLIC_LOOSE_MIDDLEWARE === "1") {
    return NextResponse.next();
  }

  const { pathname } = request.nextUrl;

  // не перевіряємо статичні/сервісні сторінки
  if (
  ...
  // якщо нема токена — на логін (для захищених)
  ...
  // якщо токен є і це auth-сторінка — на /user
  if (accessToken && isAuthPage) {
    const url = new URL("/user", request.url);
    return NextResponse.redirect(url);
  }

  // ВАЖЛИВО: НІЯКОГО verify/refresh у middleware!
  return NextResponse.next();
}
