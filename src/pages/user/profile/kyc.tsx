...
const KycApplication: React.FC = () => {
  ...
  const fetchActiveKycTemplate = async () => {
    ...
    if (error) {
      ...
      function convertBooleans(obj: Record<string, any>) {
      ...
      // Assuming data.options and data.customOptions are already JS objects
      const options = convertBooleans(templateData.options);
      const customOptions = convertBooleans(templateData.customOptions);
      setActiveTemplate({ ...templateData, options, customOptions });
      const templateLevel = Number.parseInt(router.query.l as string);
      setLevel(templateLevel);
      const generalKeys = Object.keys(options).filter((key) => {
        const field = options[key];
        return (
          !key.startsWith("document") &&
          field.enabled &&
          Number.parseInt(field.level) === templateLevel
        );
      });
      setGeneralKeys(generalKeys);

      const documentKeys = Object.keys(options).filter((key) => {
        const field = options[key];
        return (
          key.startsWith("document") &&
          field.enabled &&
  ...
  const validateCustomFields = () => {
    ...
    customFieldsFileUpload.forEach((field) => {
      ...
      if (field.required && Number.parseInt(field.level) === level) {
  ...
  const submit = async () => {
    ...
    const hasErrors =
      !(generalFieldsValid && documentsValid && customFieldsValid) ||
  ...
  return (
    <Layout color="muted" title={t("KYC Application")}>
      <div className="mx-auto max-w-2xl text-muted-800 dark:text-muted-100">
        ...
        >
            ...
            <Card className="mt-5 flex flex-col gap-5 p-5">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                  ...
                  >
                    ...
                    >
                      <div className="flex w-full items-center gap-2">
                        ...
                        <div>
                          <h5 className="font-medium font-sans text-muted-800 text-sm leading-none dark:text-muted-100">
                            {inputs[key].title}
                          </h5>
                ...
                {documentSelectionError && (
                  <div className="col-span-3 mt-2 text-danger-500 text-sm">
                    {documentSelectionError}
                  </div>
                )}
              </div>
          ...
          <div className="mt-5 flex w-full items-center justify-center">
            <Card className="flex max-w-sm items-center justify-center gap-3 p-3 text-end text-md">
