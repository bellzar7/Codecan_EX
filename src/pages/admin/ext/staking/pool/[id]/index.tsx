...
const StakingPoolEdit: React.FC = () => {
  ...
  const fetchPoolData = async () => {
    ...
    if (!error && data) {
      ...
      setType(
        ...
        ].find((type) => type.value === poolData.type) || null
      );
      setCurrency(
        formData?.currencies[poolData.type].find(
          (cur) => cur.value === poolData.currency
        ) || null
      );
      setChain(
        formData?.chains[poolData.currency]?.find(
          (chain) => chain === poolData.chain
        )
          ? { value: poolData.chain, label: poolData.chain }
          : null
      );
      setMinStake(poolData.minStake.toString());
      setMaxStake(poolData.maxStake.toString());
      setStatus(
        [
          { value: "ACTIVE", label: t("Active") },
          { value: "INACTIVE", label: t("Inactive") },
          { value: "COMPLETED", label: t("Completed") },
        ].find((status) => status.value === poolData.status) || null
      );
      setImageUrl(poolData.icon); // Set the existing image URL
    }
  };
  ...
  return (
    <Layout color="muted" title={t("Edit Staking Pool")}>
      <Card className="mb-5 p-5 text-muted-800 dark:text-muted-100">
        <div className="flex flex-col items-center justify-between md:flex-row">
          <h1 className="mb-4 text-lg md:mb-0">{t("Edit Staking Pool")}</h1>
