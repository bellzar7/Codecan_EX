...
const EditKycTemplate = () => {
  ...
  useEffect(() => {
    ...
    const fetchTemplate = async () => {
      ...
      if (!error && data) {
        ...
        const customOptions = (data as any).customOptions;
        const customOptionsArray = customOptions;
        setForm({
          ...(data as any),
          options,
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (form?.customOptions?.length >= 0) {
      const validate = form.customOptions.every(
        (option) => option.title !== ""
      );
      if (!validate) {
    ...
    if (form?.customOptions?.length >= 0) {
      ...
      if (customOptionsObject?.length >= 0) {
        values = {
          ...form,
  ...
  return (
    <Layout color="muted" title={t("Edit KYC Template")}>
