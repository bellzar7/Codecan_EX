...
export const deepParse = (obj: unknown) => {
    ...
    if (Array.isArray(value)) {
      ...
      value.forEach((item, index) => {
        if (typeof item === "object" && item !== null) {
          console.log(`  Parsing nested object at index ${index}:`);
          deepParse(item); // Recursive call
        } else {
          console.log(`  Item at index ${index}:`, item);
        }
      });
    } else if (typeof value === "object" && value !== null) {
      console.log(`  Parsing nested object for key ${key}:`);
      deepParse(value); // Recursive call
    } else {
...
export const isEditable = (
  editableCondition: Record<string, unknown> | undefined,
  values: Record<string, unknown>
): boolean => {
  ...
  return formItems
    .map((formItem: any) => {
      if (Array.isArray(formItem)) {
        return formItem.filter((nestedItem: any) =>
