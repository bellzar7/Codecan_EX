...
const normalizeTickSize = (tickSize) => {
  if (typeof tickSize !== "number" || tickSize <= 0 || Number.isNaN(tickSize)) {
    return 0.01;
  }
  return tickSize;
};
...
const aggregateData = (data, tickSize) => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const roundedPrice = roundValue(
      Math.floor(price / normalizedTickSize) * normalizedTickSize
    );
...
const processOrderBookData = (data, tickSize = 0.0001) => {
  ...
  const aggregatedAsks = aggregateData(data.asks, normalizedTickSize).sort(
    (a, b) => a.price - b.price
  );

  const aggregatedBids = aggregateData(data.bids, normalizedTickSize).sort(
    (a, b) => b.price - a.price
  );
  ...
  for (let i = 0; i < aggregatedAsks.length; i++) {
    ...
    if (ask.total > maxAskTotal) {
      maxAskTotal = ask.total;
    }
  ...
  for (let i = 0; i < aggregatedBids.length; i++) {
    ...
    if (bid.total > maxBidTotal) {
      maxBidTotal = bid.total;
    }
  ...
  const askPercentage =
    totalVolume > 0
      ? roundValue((totalAskVolume / totalVolume) * 100, 2).toFixed(2)
      : "0.00";
  const bidPercentage =
    totalVolume > 0
      ? roundValue((totalBidVolume / totalVolume) * 100, 2).toFixed(2)
      : "0.00";
