...
const _HIGHLOAD_WALLET_TIMEOUT = 60 * 60; // 1 hour
...
class TonService {
  ...
  private formatAddress(address): string {
    ...
    const _isTestnet = network === "testnet";
  ...
  private parseTonTransactions(rawTransactions: any[]): ParsedTransaction[] {
    return rawTransactions.map((tx) => {
      ...
      const amount = (Number.parseInt(in_msg.value, 10) / 1e9).toString(); // Convert from nanoTONs to TONs
  ...
  public async handleTonWithdrawal(
    ...
    try {
      ...
      if (seqno === null || seqno === undefined || Number.isNaN(seqno)) {
        seqno = 0;
      }
      ...
      let retries = 0;
      const maxRetries = 10;
      const retryDelay = 10_000; // 10 seconds delay between retries
