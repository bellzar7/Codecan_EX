...
class MoneroService {
  private readonly daemonRpcUrl: string;
  private readonly walletRpcUrl: string;
  private readonly rpcUser: string | undefined;
  private readonly rpcPassword: string | undefined;
  private readonly walletPassword: string | undefined;
  ...
  private static readonly RETRY_INTERVAL = 60_000; // 1 minute
      ...
      await MoneroService.addToQueue(async () => {
        ...
        if (!MoneroService.processedTransactions.has(transactionHash)) {
          ...
          if (existingTransaction) {
            ...
            if (transactionInfo.result?.transfer) {
  ...
  public async ensureWalletExists(walletName: string): Promise<void> {
    try {
      ...
      if (openResponse.error?.message.includes("Failed to open wallet")) {
  ...
  public async estimateMoneroFee(
    priority = 1 // Transaction priority: 0 (slow), 1 (normal), 2 (fast), 3 (fastest)
  ): Promise<number> {
  ...
  public async handleMoneroWithdrawal(
    ...
    amount: number,
    toAddress: string,
    priority = 0 // Transaction priority: 0 (low), 1 (medium), 2 (high)
  ): Promise<void> {
