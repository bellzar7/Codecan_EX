...
class SolanaService {
  ...
  private constructor(cacheExpirationMinutes = 30) {
  ...
  public async monitorSPLTokenDeposits(
    ...
    try {
      ...
      // Inactivity timeout handler
      let inactivityTimeout: NodeJS.Timeout | undefined | any = setTimeout(
        async () => {
      ...
      programChangeSubscriptionId = this.connection.onProgramAccountChange(
        ...
        async ({ accountId, accountInfo }, context) => {
          try {
            ...
            if (!blockTransactions?.transactions) {
              console.error(
  ...
  async transferSol(
    ...
    try {
      ...
      if (txResult?.meta && txResult.meta.err === null) {
  ...
  async handleSplTokenWithdrawal(
    ...
    try {
      ...
      const amountInLamports = Math.round(amount * 10 ** decimals);
  ...
  public async mintInitialSupply(
    ...
    try {
      ...
      const mintAmount = initialSupply * 10 ** decimals;
