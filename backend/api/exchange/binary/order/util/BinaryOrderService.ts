...
const _binaryTurboProfit = validateBinaryProfit(
...
export class BinaryOrderService {
  ...
  static async createOrder({
    ...
    return await sequelize.transaction(async (t) => {
      ...
      BinaryOrderService.scheduleOrderProcessing(finalOrder, userId);
  ...
  static async processOrder(userId: string, orderId: string, symbol: string) {
    try {
      ...
      if (
        ...
        touched = await BinaryOrderService.checkIfBarrierTouched(
          exchange,
          order.symbol,
          order.createdAt,
          order.closedAt,
      ...
      if (
        ...
        turboBreached = await BinaryOrderService.checkTurboBarrierBreach(
          exchange,
          order.symbol,
          order.createdAt,
          order.closedAt,
      ...
      const updateData = BinaryOrderService.determineOrderStatus(
        order,
        closePrice,
        touched,
        turboBreached
      ...
      await BinaryOrderService.updateBinaryOrder(orderId, updateData);
      BinaryOrderService.orderIntervals.delete(orderId);
    } catch (error) {
  ...
  static async checkTurboBarrierBreach(
    ...
    try {
      while (!breached && from < until) {
        ...
        const lastCandleTime = ohlcv.at(-1)[0];
  ...
  static async cancelOrder(
    ...
    await BinaryOrderService.processStandardCancel(
      order,
      currentPrice,
      percentage
    );
  ...
  private static async processStandardCancel(
    ...
    await sequelize.transaction(async (t) => {
      ...
      if (BinaryOrderService.orderIntervals.has(order.id)) {
        clearTimeout(BinaryOrderService.orderIntervals.get(order.id));
        BinaryOrderService.orderIntervals.delete(order.id);
      }
  ...
  static async processPendingOrders() {
    try {
      ...
      const unmonitoredOrders = pendingOrders.filter((order) => {
        ...
        return (
          closedAtTime <= currentTime &&
          !BinaryOrderService.orderIntervals.has(order.id)
        );
      });
      ...
      for (const order of unmonitoredOrders) {
        ...
        await BinaryOrderService.updateBinaryOrder(order.id, updateData);
      }
    } catch (error) {
  ...
  private static scheduleOrderProcessing(
    ...
    if (delay < 0) {
      ...
      BinaryOrderService.processOrder(userId, order.id, order.symbol);
      return;
    }
    ...
    BinaryOrderService.orderIntervals.set(order.id, timer);
  }
}
