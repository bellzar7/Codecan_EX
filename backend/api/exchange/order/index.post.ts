...
import { isSpotEngineInitialized, placeOrder } from "@b/utils/spot";
import {
  formatWaitTime,
  handleBanStatus,
  loadBanStatus,
...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Pre-existing complex order creation logic with multiple validation steps
export default async (data: Handler) => {
  ...
  try {
    ...
    const formattedAmount = Number.parseFloat(amount.toFixed(amountPrecision));
    const formattedPrice = Number.parseFloat(
      orderPrice.toFixed(pricePrecision)
    );
    ...
    // Step 11: Use internal matching engine if initialized, otherwise fall back to external API
    if (isSpotEngineInitialized()) {
      // Use internal SPOT matching engine
      try {
        ...
        const orderData =
          ...
          (result.filledOrders.length > 0 ? result.filledOrders[0] : null);
        ...
        if (orderData) {
          ...
          addOrderToTrackedOrders(user.id, {
            ...
            timestamp: orderData.createdAt.getTime(),
          });
        } else if (result.trades.length > 0) {
          // Order was fully filled and removed, get from trades
        ...
        console.log("Internal matching engine error:", error);
        throw new Error(
...
const mapOrderData = (order: OrderDataInput) => {
  return {
