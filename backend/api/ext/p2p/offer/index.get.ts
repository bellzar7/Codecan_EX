...
export default async (data: Handler) => {
  ...
  const formattedData = await getFiltered({
  ...
  formattedData.items = formattedData.items.filter((item: any) => {
    if (item?.avatarToDisplay && item?.user) {
      item.user.avatar = item.avatarToDisplay;
    }
    if (item?.userIds?.length > 0) {
      return item.userIds.some((offer: any) => offer?.value === user?.id);
    }
    return true;
  });
