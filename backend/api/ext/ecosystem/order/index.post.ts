...
export default async (data: any) => {
  ...
  try {
    ...
    if (
      !(market.metadata.precision?.amount && market.metadata.precision.price)
    ) {
      throw createError({
        statusCode: 422,
    ...
    if (market?.metadata?.maker < 0 || market?.metadata?.taker < 0) {
      throw createError({
        statusCode: 422,
    ...
    if (type.toLowerCase() === "market") {
      let bestPrice = await getBestPriceFromOrderBook(symbol, side);
      if (!bestPrice) {
        bestPrice = 1;
      }
      if (!bestPrice) {
    ...
    if (!effectivePrice || Number.isNaN(effectivePrice)) {
      throw createError({
        statusCode: 422,
    ...
    const cost = Number.parseFloat(costCalculated.toFixed(precision));

    if (side.toUpperCase() === "BUY" && (Number.isNaN(cost) || cost <= 0)) {
      throw createError({
        statusCode: 422,
    ...
    if (side.toUpperCase() === "SELL") {
      if (
        !currencyWallet ||
        Number.parseFloat(currencyWallet.balance.toString()) < amount
      ) {
        throw createError({
          statusCode: 400,
      ...
      if (
        !pairWallet ||
        Number.parseFloat(pairWallet.balance.toString()) < cost
      ) {
        throw createError({
          statusCode: 400,
