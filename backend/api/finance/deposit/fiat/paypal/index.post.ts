...
export default async (data: Handler) => {
  ...
  const taxAmount = Math.max(
    (Number.parseFloat(amount) * percentageFee) / 100 + fixedFee,
    0
  );

  const itemAmount = Number.parseFloat(amount) - taxAmount;
  if (itemAmount < 0) {
    throw new Error("Invalid amount");
  }
  const totalAmount = Number.parseFloat(amount).toFixed(2); // Total amount includes tax
