...
async function checkScyllaService() {
  ...
  const scyllaHost = process.env.SCYLLA_HOST;
  const scyllaPassword = process.env.SCYLLA_PASSWORD;

  if (
    !(scyllaUsername && scyllaPassword) ||
    scyllaUsername === "" ||
    scyllaPassword === ""
  ) {
    return {
  ...
  const client = new Client({
    contactPoints: [scyllaHost ? scyllaHost : "localhost"], // Replace with your Scylla DB contact points
  ...
  try {
    ...
    const _result = await client.execute("SELECT now() FROM system.local");
...
async function checkEthereumService() {
  ...
  if (
    !(ethExplorerApiKey && ethRpcMap[ethNetwork]) ||
    ethExplorerApiKey === "" ||
...
async function checkBscService() {
  ...
  if (
    !(bscExplorerApiKey && bscRpcMap[bscNetwork]) ||
    bscExplorerApiKey === "" ||
...
async function checkPolygonService() {
  ...
  if (
    !(polygonExplorerApiKey && polygonRpcMap[polygonNetwork]) ||
...
async function checkFtmService() {
  ...
  if (
    !(ftmExplorerApiKey && ftmRpcMap[ftmNetwork]) ||
    ftmExplorerApiKey === "" ||
...
async function checkOptimismService() {
  ...
  if (
    !(optimismExplorerApiKey && optimismRpcMap[optimismNetwork]) ||
...
async function checkArbitrumService() {
  ...
  if (
    !(arbitrumExplorerApiKey && arbitrumRpcMap[arbitrumNetwork]) ||
...
async function checkCeloService() {
  ...
  if (
    !(celoExplorerApiKey && celoRpcMap[celoNetwork]) ||
    celoExplorerApiKey === "" ||
...
async function checkEvmNetwork(serviceName, rpc, wss, network) {
  ...
  try {
    ...
    if (
      rpc ===
      "https://bnb-testnet.g.alchemy.com/v2/HReGk81mXDwXvuG7Q9SIXSuLk_fzYlHu"
    ) {
      network = {
        name: "bnbTestnet",
        chainId: 97,
