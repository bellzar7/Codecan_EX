...
import type { IErrorHandler, IRequestHandler } from "./types";
import {
  allowedOrigins,
  serveStaticFile,
  setCORSHeaders,
...
import { startEcoBridge } from "@b/integrations/twelvedata/bridge";
...
import { initializeSpotEngine } from "@b/utils/spot";
...
export class MashServer extends RouteHandler {
  ...
  private roles: any;
  private readonly benchmarkRoutes: { method: string; path: string }[] = [];
  ...
  public async initializeServer() {
    try {
      ...
      console.log(
        `\x1b[36m${threadType}: Initializing TWD exchange provider...\x1b[0m`
      );
      await this.safeExecute(
        () => ensureTwdExchangeProvider(),
        "ensureTwdExchangeProvider"
      );
      ...
      await this.safeExecute(async () => {
        ...
        const _extensions = await cacheManager.getExtensions();
      ...
      console.log(
        `\x1b[36m${threadType}: Initializing SPOT trading engine...\x1b[0m`
      );
      await this.safeExecute(
  ...
  public cors() {
    // PROD MODE: allow only from allowlist
    ...
    this.app.options("/*", (res, req) => {
      ...
      // у деві дозволяємо будь-який origin (дзеркалимо). У проді — тільки з allowlist.
      const allow =
        (isDev && origin) || (origin && allowedOrigins.includes(origin)) || "";

      if (allow) {
        setCORSHeaders(res, allow);
      }
