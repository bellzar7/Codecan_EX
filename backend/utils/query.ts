...
export async function getFiltered({
  ...
  customStatus?.forEach(({ key, true: trueValue, false: falseValue }) => {
    if (Object.hasOwn(directFilters, key)) {
  ...
  Object.entries(directFilters as { [key: string]: any }).forEach(
    ([key, filterValue]) => {
      if (numericFields.includes(key) && typeof filterValue !== "object") {
        ...
        whereClause[key] = Number.parseFloat(filterValue) || filterValue;
  ...
  const modifiedRows = [] as any;
  if (rows?.length > 0 && !raw) {
    for (const row of rows) {
      if (row?.dataValues?.messages) {
        const messages = [...row?.dataValues?.messages];
        row.messages = null;

        const resp = row.get({ plain: true }) as any;
        resp.messages = messages;
        modifiedRows.push(resp);
      } else {
        const resp = row.get({ plain: true }) as any;
        modifiedRows.push(resp);
      }
  ...
  const resp = modifiedRows?.length ? modifiedRows : rows;
...
export function parseFilterParam(
  ...
  Object.entries(filtersObject as { [key: string]: any }).forEach(
    ([key, value]) => {
      ...
      const isNumericField = lastKey ? numericFields.includes(lastKey) : false;
...
export async function getRecord<T extends Model>(
  ...
  if (data?.dataValues?.messages) {
    messages = [...data?.dataValues?.messages];
    data.messages = null;
  }
  ...
  const resp = data?.get({ plain: true }) as any;
  if (messages?.length) {
    resp.messages = messages;
  }
  return resp;
...
export async function storeRecord({
  ...
  try {
    ...
    if (
      data.customAddressWalletsPairFields === undefined ||
      data.customAddressWalletsPairFields === null
    ) {
      data.customAddressWalletsPairFields = [];
    }
    if (
      data.customRestrictionPairFields === undefined ||
      data.customRestrictionPairFields === null
    ) {
      data.customRestrictionPairFields = [];
    }
    ...
    // Ensure customFields is an array
    if (!Array.isArray(data.customAddressWalletsPairFields)) {
      throw new Error("customAddressWalletsPairFields must be an array");
    }
    if (!Array.isArray(data.customRestrictionPairFields)) {
      throw new Error("customRestrictionPairFields must be an array");
    }
