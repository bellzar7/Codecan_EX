    ...
    if (tx.data === "0x") {
      ...
      switch (methodID) {
        case "0xa9059cbb": {
          ...
          const amount = Number.parseInt(tx.data.substring(74, 138), 16);
...
export async function cacheTokenDecimals() {
  try {
    ...
    await redis.setex("token_decimals", 86_000, JSON.stringify(tokenDecimals));
  } catch (error) {
...
function getConversionFactor(chain) {
  return {
    BTC: 100_000_000, // For Bitcoin
    LTC: 100_000_000, // For Litecoin
    DOGE: 100_000_000, // For Dogecoin (adjust if different)
    DASH: 100_000_000, // For Dash
  }[chain];
}
...
export const dashNetwork = {
  ...
  bech32: "dash",
  bip32: {
    public: 0x02_fe_52_f8,
    private: 0x02_fe_52_cc,
  },
  pubKeyHash: 0x4c,
  scriptHash: 0x10,
  wif: 0xcc,
