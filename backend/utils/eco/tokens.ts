...
export const fetchTokenHolders = async (
  ...
  try {
    ...
    for (const tx of data.result) {
      ...
      holders[from] = (holders[from] || 0) - Number.parseFloat(value);
      holders[to] = (holders[to] || 0) + Number.parseFloat(value);
    }
    ...
    const formattedHolders = Object.entries(holders)
      .map(([address, balance]) => ({
        address,
        balance: Number.parseFloat((balance / 10 ** decimals).toFixed(8)),
      }))
