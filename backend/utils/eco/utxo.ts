...
export const watchAddressBlockCypher = (chain, address, callback) => {
  ...
  ws.on("open", function open() {
    ws.send(JSON.stringify({ event: "unconfirmed-tx", address }));
  });
...
export async function createTransactionDetailsForUTXO(
  ...
  const txDetails = {
    ...
    chain,
    hash: txHash,
    from: inputs.flatMap((input) => input.addresses),
    to: outputs.flatMap((output) => output.addresses),
    amount,
    inputs,
    outputs,
  };
...
export const fetchUtxoTransaction = async (txHash, chain) => {
  ...
  const retryDelay = 30_000; // 30 seconds delay between retries
...
export async function handleUTXOWithdrawal(transaction: Transaction) {
  ...
  function getDustThreshold(chain: string): number {
    switch (chain) {
      ...
      case "DOGE":
        return 100_000_000; // DOGE has different units
      case "DASH":
...
async function recordChangeUtxo(txid, changeAmount, wallet, chain) {
  ...
  const changeOutput = changeTxData.outputs.find((output) =>
    output.addresses?.includes(changeAddress)
  );
