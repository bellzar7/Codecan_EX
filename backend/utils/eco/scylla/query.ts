  ...
  try {
    ...
    const sortedRows = result?.rows
      ?.sort((a, b) => {
        ...
        // Перша перевірка — якщо обидва є Date-об’єктами
        if (
          fieldA instanceof Date &&
          fieldB instanceof Date &&
          typeof fieldA.getTime === "function" &&
          typeof fieldB.getTime === "function"
        ) {
          return query?.sortOrder === "asc"
        ...
        // Друга перевірка — якщо обидва це валідні дати у вигляді рядків
        if (
          typeof fieldA === "string" &&
          typeof fieldB === "string" &&
          fieldA &&
          fieldB &&
          !Number.isNaN(Date.parse(fieldA)) &&
          !Number.isNaN(Date.parse(fieldB))
        ) {
