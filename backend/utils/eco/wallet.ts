...
export async function getWalletByUserIdAndCurrency(userId, currency) {
  ...
  if (
    ...
    const tokensWithoutAddress = tokens.filter(
      (token) => !(addresses && Object.hasOwn(addresses, token.chain))
    );
...
export const storeWallet = async (user, currency) => {
  ...
  return await sequelize.transaction(async (transaction) => {
    ...
    let addresses = existingWallet?.address
...
const handleNoPermitContract = async (token, _newWallet, addresses) => {
  addresses[token.chain] = {
    balance: 0,
  };
};
...
export const executeNoPermitWithdrawal = async (
  ...
  try {
    ...
    let tokenOwner, custodialContract, _custodialContractAddress;
    for (const custodialWallet of custodialWallets) {
      ...
      if (BigInt(balance) >= amount) {
        ...
        _custodialContractAddress = custodialWallet.address;
        break;
      }
    }
    if (!tokenOwner) {
...
export const handleEcosystemDeposit = async (trx) => {
  try {
    ...
    chainAddress.balance =
      (chainAddress.balance || 0) + Number.parseFloat(trx.amount);

    const walletBalance = wallet.balance + Number.parseFloat(trx.amount);
    ...
    if (utxoChains.includes(trx.chain)) {
      ...
      const totalInputValue = trx.inputs.reduce((sum, input) => {
        return sum + satoshiToBTC(Number.parseFloat(input.value));
      }, 0);

      const totalOutputValue = trx.outputs.reduce((sum, output) => {
        return sum + satoshiToBTC(Number.parseFloat(output.value));
      }, 0);
      ...
      if (
        Number.isNaN(Number.parseFloat(trx.gasUsed)) ||
        Number.isNaN(Number.parseFloat(trx.gasPrice))
      ) {
        fee = 0;
      } else {
        fee = Number.parseFloat(trx.gasUsed) * Number.parseFloat(trx.gasPrice);
      }
    }
    ...
    const createdTransaction = await models.transaction.create({
      ...
      amount: Number.parseFloat(trx.amount),
...
const updateBalancePrecision = (balance, chain) => {
  ...
  if (fixedPrecisionChains[chain] !== undefined) {
    return Number.parseFloat(balance.toFixed(fixedPrecisionChains[chain]));
  }
  return balance;
};
