...
import { startOfDay, startOfHour, startOfMinute, startOfWeek } from "date-fns";
...
export function intervalToMs(interval: string): number {
  ...
  const value = Number.parseInt(interval.slice(0, -1), 10);
...
export async function convertToOrderArray(
  ...
  for (let i = 0; i < rawData.length; i += 2) {
    parsedData.push([
      Number.parseFloat(rawData[i]),
      Number.parseFloat(rawData[i + 1]),
    ]);
  }
  return parsedData;
}
