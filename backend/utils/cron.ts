...
import { processTwdLimitOrders } from "./crons/twdOrder";
import {
  ...
  private loadNormalCronJobs() {
    this.cronJobs.push(
      ...
      },
      {
        ...
        description:
          "Executes TWD paper trading LIMIT orders when price matches.",
...
async function updateRatesFromData(exchangeRates) {
  ...
  for (const currency of currencies) {
    if (Object.hasOwn(exchangeRates, currency.id)) {
  ...
  try {
    ...
    const bulkUpdateData = usdtPairs
      .map((symbol) => {
        ...
        if (matchingCurrency) {
          matchingCurrency.price = Number.parseFloat(price); // update price directly in the object
