...
import type { Candle } from "@b/utils/eco/scylla/queries";
...
export class FuturesMatchingEngine {
  ...
  private readonly orderQueue: Record<string, FuturesOrder[]> = {};
  private readonly marketsBySymbol: Record<string, any> = {};
  private readonly lockedOrders: Set<string> = new Set();
  private readonly lastCandle: Record<string, Record<string, Candle>> = {};
  private readonly yesterdayCandle: Record<string, Candle> = {};

  public static getInstance(): Promise<FuturesMatchingEngine> {
    if (!FuturesMatchingEngine.instancePromise) {
      FuturesMatchingEngine.instancePromise = (async () => {
        const instance = new FuturesMatchingEngine();
        await instance.init();
        return instance;
      })();
    }
    return FuturesMatchingEngine.instancePromise;
  }
  ...
  public async addToQueue(order: FuturesOrder) {
    ...
    if (
      ...
      !order.updatedAt ||
      Number.isNaN(new Date(order.updatedAt).getTime())
    ) {
      logError(
        "matching_engine",
        new Error("Invalid date in order"),
  ...
  private updateLastCandles(
    ...
    if (
      trades &&
      trades.length > 0 &&
      (trades.at(-1) as any).price !== undefined
    ) {
      finalPrice = toBigIntFloat((trades.at(-1) as any).price);
  ...
  private generateCandleQueries(
    ...
    const normalizedCurrentTime = normalizeTimeToInterval(Date.now(), interval);
    ...
    if (shouldCreateNewCandle) {
      ...
      const normalizedTime = new Date(
        normalizeTimeToInterval(Date.now(), interval)
      );
