...
export async function createPosition(
  ...
  const query = `
    INSERT INTO ${scyllaFuturesKeyspace}.position (id, "userId","orderId", symbol, side, "entryPrice", amount, leverage, "unrealizedPnl", "stopLossPrice", "takeProfitPrice", status, "createdAt", "updatedAt")
    VALUES (?, ?,?, ?, ?, ?, ?, ?, ?, ?, ?, 'OPEN', ?, ?);
  `;
  const params = [
    makeUuid(),
...
export async function updatePositionInDB(
  ...
  const query = `
    ...
    SET "entryPrice" = ?, amount = ?, "unrealizedPnl" = ?, "stopLossPrice" = ?, "takeProfitPrice" = ?, "updatedAt" = ?, "createdAt" = ?, "leverage" = ?
    WHERE "userId" = ? AND id = ?;
  `;
  const params = [
    entryPrice.toString(),
