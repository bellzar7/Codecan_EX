...
export async function getOrderByUserIdAndOrderId(
  userId: string,
  id: string
  ...
  try {
    ...
    console.error(`Failed to fetch futures order: ${error.message}`);
    throw new Error(`Failed to fetch futures order: ${error.message}`);
  }
}
