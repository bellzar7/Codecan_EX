...
class ExchangeManager {
  ...
  // Non-CCXT providers that should be excluded from SPOT trading
  private static readonly NON_CCXT_PROVIDERS = ["twelvedata", "twd"];

  private async fetchActiveProvider(): Promise<string | null> {
    try {
      // Find active CCXT provider (exclude TwelveData and other non-CCXT providers)
      ...
      // If the active provider is non-CCXT (e.g., TwelveData), find a CCXT provider
      if (
        ExchangeManager.NON_CCXT_PROVIDERS.includes(provider.name.toLowerCase())
      ) {
        // Try to find an active CCXT provider
        const ccxtProvider = await models.exchange.findOne({
          where: {
            status: true,
            name: {
              [Op.notIn]: ExchangeManager.NON_CCXT_PROVIDERS.map((p) =>
                p.toLowerCase()
              ),
            },
          },
        ...
        if (ccxtProvider) {
          return ccxtProvider.name;
        }

        // No CCXT provider found, return null for SPOT operations
        return null;
      }
  ...
  private async initializeExchange(
    ...
    // Skip non-CCXT providers (TwelveData, custom integrations, etc.)
    const nonCcxtProviders = ["twelvedata", "twd"];
    if (nonCcxtProviders.includes(provider.toLowerCase())) {
      console.log(`[ExchangeManager] Skipping non-CCXT provider: ${provider}`);
      return null;
    }
  ...
  private async handleRateLimitError(_provider: string): Promise<void> {
    const banTime = Date.now() + 60_000; // Ban for 1 minute
    await saveBanStatus(banTime);
    await sleep(60_000); // Wait for 1 minute
  }
