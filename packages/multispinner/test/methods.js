...
describe("Multispinner methods", () => {
  ...
  describe("constructor", () => {
    ...
    it("spinners do not leak between instances", () => {
      ...
      s1.map((s) => assert.isFalse(Object.hasOwn(m2.spinners, s)));
      s2.map((s) => assert.isFalse(Object.hasOwn(m1.spinners, s)));
    });
  ...
  describe("loop", () => {
    ...
    it("step through spinner animation frames and update prop", () => {
      ...
      while (i < m.frameCount) {
        assert.equal(m.symbol[states.incomplete], m.frames[i], `frame ${i}`);
    ...
    describe("update current spinner strings", () => {
      ...
      function testCase(state, action) {
        ...
        const expected = chalk[m.color[state]](
          [m.indentStr, m.symbol[state], " ", m.spinners[spinner].text].join("")
        );
    ...
    describe("if allCompleted is true", () => {
      ...
      beforeEach(() => {
        allCompletedStub = sinon.stub(m, "allCompleted").returns(true);
      ...
      it('emit "success" if allSuccess is true', () => {
        const allSuccessStub = sinon.stub(m, "allSuccess").returns(true);
        let eventEmitted = false;
